...
async def recv_msg_ping(ws, raddr):
    ...
    while msg is None:
        try:
            ...
            print('Signaling server: Sending keepalive ping to {!r} in recv'.format(raddr))
            await ws.ping()
    return msg
...
async def cleanup_session(uid):
    if uid in sessions:
        ...
        del sessions[uid]
        print("Signaling server: Cleaned up {} session".format(uid))
        if other_id in sessions:
            del sessions[other_id]
            print("Signaling server: Also cleaned up {} session".format(other_id))
            # If there was a session with this peer, also
            # close the connection to reset its state.
            if other_id in peers:
                print("Signaling server: Closing connection to {}".format(other_id))
                wso, oaddr, _ = peers[other_id]
                del peers[other_id]
                await wso.close(reason="Clean up session for id {!r} (closing {!r})".format(other_id))
...
async def remove_peer(uid):
    ...
    if uid in peers:
        ...
        del peers[uid]
        await ws.close(reason="remove peer {!r} ".format(uid))
        print("Signaling server:: Disconnected from peer {!r} at {!r}".format(uid, raddr))
...
async def connection_handler(ws, uid):
    ...
    print("Signaling server: Registered peer {!r} at {!r}".format(uid, raddr))
    while True:
        ...
        if peer_status is not None:
            ...
            if peer_status == 'session':
                ...
                print("Signaling server: {} -> {}: {}".format(uid, other_id, msg))
                await wso.send(msg)
            # We're in a room, accept room-specific commands
                ...
                if msg.startswith('ROOM_PEER_MSG'):
                    ...
                    print('Signaling server: room {}: {} -> {}: {}'.format(room_id, uid, other_id, msg))
                    await wso.send(msg)
                    ...
                    print('Signaling server: room {}: -> {}: {}'.format(room_id, uid, msg))
                    await ws.send(msg)
                else:
        ...
        # Requested a session with a specific peer
            ...
            print('Signaling server: Session from {!r} ({!r}) to {!r} ({!r})'
...
async def handler(ws, path):
    ...
    '''
    raddr = ws.remote_address
    print("Signaling server: Connected to {!r}".format(raddr))
    peer_id = await hello_peer(ws)
    try:
        await connection_handler(ws, peer_id)
    except websockets.ConnectionClosed:
        print("Signaling server: Connection to peer {!r} closed, exiting handler".format(raddr))
    finally:
        await remove_peer(peer_id)

# Create an SSL context to be used by the websocket server
certpath = options.cert_path
print('Signaling server: Using TLS with keys in {!r}'.format(certpath))
...
try:
    ...
    print("Signaling server: Certificates not found, did you run generate_cert.sh?")
    print("Signaling server: ssl will be disabled")



print("Signaling server: Listening on https://{}:{}".format(*ADDR_PORT))
# Websocket server
